...
const register = (data) => {
  ...
  return new Promise((resolve, reject) =>
    Pool.query(
      `INSERT INTO users(id_user,email_user,password_user,name_user,phone_number,alamat,verif,otp)VALUES('${id_user}','${email_user}','${password_user}','${name_user}','${phone_number}','${alamat}',0,'${otp}')`,
      (err, result) => {
        if (!err) {
          resolve(result);
        } else {
